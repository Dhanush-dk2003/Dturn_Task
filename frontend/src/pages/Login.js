...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (loggedInUser) {
      ...
      else if (loggedInUser.role === "USER") navigate("/userdashboard");
      else navigate("/");
    } else {
  ...
  return (
    <div className="w-100 h-100 d-flex align-items-center justify-content-center min-vh-100 bg-body-secondary animate__animated animate__fadeIn">
      <div
        className="p-4 bg-white rounded-5 shadow-lg border border-0 position-absolute top-50 start-50 translate-middle"
        style={{ maxWidth: "380px", width: "100%" }}
      >
        <h4 className="text-center text-primary fw-bold mb-4">Welcome Back</h4>
        {error && (
          <div className="alert alert-danger text-center py-2 px-3">
            {error}
          </div>
        )}
        <form onSubmit={handleSubmit} noValidate>
          <div className="input-group mb-3">
            ...
            />
            {validation.email && (
              <div className="invalid-tooltip d-block">{validation.email}</div>
            )}
          </div>
          <div className="input-group mb-3">
            <span className="input-group-text">
          ...
          <button className="btn btn-primary w-100 rounded-4 fw-semibold">
            Login
          </button>
        </form>
